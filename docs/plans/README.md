...
- `jeong-doeun-plan.md`: gameplay core, in-game overlay 오너 계획
...
flowchart TD
    ...
    B --> E["5. In-Game UI Layer\n InGameOverlaySlot (후속 분리: HUD/EventAlert/EventLog)"]
...
- 정도은: `types`, `store`, `systems`, `data`, `InGameOverlaySlot`(후속 HUD 계열 포함)
- 윤영서: `RaceScreen`, `Track`, `Character`, `Environment`, `CameraSystem`, `SpeechBubble`
- 윤영서는 overlay를 `RaceScreen.tsx`에 직접 붙이지 않는다.
...
- `routes/race.tsx`는 현재 `RaceRouteComposition`을 렌더링하고, 내부에서 `RaceSceneSlot`과 `InGameOverlaySlot`을 조합한다.
- `SpeechBubble`은 scene anchored element라 `RaceScreen` 내부에서 렌더링한다.
- 정도은은 `RaceScreen.tsx`를 수정하지 않고 overlay 계열만 완성한다.
- 윤영서는 `InGameOverlaySlot.tsx`를 수정하지 않는다.
...
5. 정도은이 `InGameOverlaySlot`을 기준으로 오버레이 UI를 붙인다.
