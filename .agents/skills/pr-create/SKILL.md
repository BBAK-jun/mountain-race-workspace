---
name: pr-create
description: Draft a pull request for the mountain race workspace. Use when turning the current branch diff into a reviewable PR title and body that covers apps/web, apps/api, docs, Cursor config, Codex config, CI, deployment, verification, and remaining risk.
---
...
- Writing PRs that touch app code, docs, `.cursor`, `.codex`, `.agents`, CI, or deployment files
...
3. Choose a PR title that matches existing history:
   ...
   - Examples: `feat(web): add race result shell`, `chore(codex): port cursor skills`
...
- Do not treat `.cursor`, `.codex`, `.agents`, or workflow changes as invisible tooling noise; explain how they affect contributors or release flow.
