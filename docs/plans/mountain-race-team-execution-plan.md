...
- 인게임은 정도은과 윤영서가 맡되, 한 명은 `state/system/overlay`, 다른 한 명은 `scene/camera`만 맡는다.

### 원칙 3. 인게임 두 사람은 같은 파일을 수정하지 않는다

- 정도은은 `types`, `store`, `systems`, `data`, `overlay` 계열만 수정한다.
...
- 정도은: `GameState`, action 이름, 인게임 overlay가 읽을 state 초안 작성
...
- 오버레이는 현재 `InGameOverlaySlot`에서 시작하고, 이후 HUD/이벤트 로그 실컴포넌트로 세분화한다.
...
- (필요 시) `HUD`/`EventAlert`/`EventLog` 분리
...
- race scene과 in-game overlay는 박준형의 `routes/race.tsx`(현재 `RaceRouteComposition`)에서만 합친다.
...
- race 진행 중 rank와 in-game overlay 정보가 서로 일치한다.
...
- `InGameOverlaySlot`(및 후속 HUD 계열), `types`, `store`, `systems`, `data`는 정도은만 수정
