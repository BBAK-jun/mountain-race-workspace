...
당신은 이 게임이 “레이스처럼 보이게” 만드는 사람이다. 캐릭터가 트랙을 따라 움직이고, 카메라가 그걸 제대로 잡아줘야 HUD와 이벤트 로그도 의미가 생긴다.
...
- HUD를 빼도 scene만 보면 race 진행이 이해된다.
...
- `HUD.tsx`, `EventAlert.tsx`, `EventLog.tsx`를 직접 수정하지 않는다.
